  ...
  if (isError) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-gray-600">There was an error loading the traceability requests. Please try again later.</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        ...
        <Link href="/traceability/request/create">
          ...
          <table className="w-full min-w-[1200px]">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="text-left py-4 px-4 font-medium text-gray-900 w-32">Progress</th>
                  ...
                  <td className="py-4 px-4">
                    <div className="flex flex-col">
                      <span className="text-gray-900">
                        {request.suppliers.length > 0 ? `${request.suppliers.length} suppliers` : 'No suppliers'}
                      </span>
                      <span className="text-gray-500 text-sm">
                        {request.suppliers.length > 1 ? `+${request.suppliers.length - 1} more suppliers` : ''}
                      </span>
                    </div>
                  </td>
                  <td className="py-4 px-4">
                  ...
                  <td className="py-4 px-4">
                    <div className="flex flex-col">
                      <span className="text-sm text-gray-900">
                        {request.progress.responded}/{request.progress.total} responded
                      </span>
                      <div className="w-16 bg-gray-200 rounded-full h-1.5 mt-1">
                        <div 
                          ...
                          style={{ 
                            width: `${(request.progress.responded / request.progress.total) * 100}%` 
                          }}
                        ></div>
                      </div>
                    </div>
      ...
      <div className="flex items-center justify-between">
        <div className="text-sm text-gray-700">
          Showing <span className="font-medium">1</span> to <span className="font-medium">{requests?.length || 0}</span> of <span className="font-medium">{requests?.length || 0}</span> results
        </div>
