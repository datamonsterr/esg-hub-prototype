...
import { Search, Database, Upload, Webhook, Package } from "lucide-react"
import { Button } from "@/src/components/ui/button"
import { Input } from "@/src/components/ui/input"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/src/components/ui/select"
import { Card, CardContent, CardDescription, CardFooter, CardHeader, CardTitle } from "@/src/components/ui/card"
import { Badge } from "@/src/components/ui/badge"
import { useGetDataIntegrations } from '@/src/api/integration';
import { GlobalLoading } from '@/src/components/global-loading';
import { IntegrationMethod, PopularIntegration } from '@/src/types/integration';
...
export function DataIntegrations({ selectedProduct, onProductSelect, onStartUpload }: {
  ...
  const { dataIntegrations, isLoading, isError } = useGetDataIntegrations();
  ...
  return (
    <div className="space-y-8">
      <div className={`flex gap-4 ${!selectedProduct ? "opacity-50 pointer-events-none" : ""}`}>
